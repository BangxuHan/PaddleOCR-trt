...
class TextDetector(object):
    ...
    def getPolygonArea(self, dt_boxes):
        list1 = []
    ...
    def getPolygonCertroid(self, points):
        x_sum, y_sum = 0., 0.
        for point in points:
            x_sum += point[0]
            y_sum += point[1]
    ...
    def defineOtherPosition(self, dt_boxes, index, position):
        number_box = dt_boxes[index]
        number_centroid = TextDetector.getPolygonCertroid(self, number_box)

        other_boxes = np.delete(dt_boxes, index, axis=0)
        # print(other_boxes)
        top_left, top_right, bot_left, bot_right = [number_box], [number_box], [number_box], [number_box]
        for point in other_boxes:
            point_centroid = TextDetector.getPolygonCertroid(self, point)
            if point_centroid[0] < number_centroid[0] and point_centroid[1] < number_centroid[1]:
                top_left.append(point)
            if point_centroid[0] > number_centroid[0] and point_centroid[1] < number_centroid[1]:
                top_right.append(point)
            if point_centroid[0] < number_centroid[0] and point_centroid[1] > number_centroid[1]:
                bot_left.append(point)
            if point_centroid[0] > number_centroid[0] and point_centroid[1] > number_centroid[1]:
                bot_right.append(point)
        if position == 0:
            return top_left
        if position == 1:
            return top_right
